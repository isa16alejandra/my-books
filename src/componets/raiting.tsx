...
function Rating(props: any) {
  const { isEmoji, rating, setRating, noChange } = props;
  const EMOJIS = [
    <path
      fill="currentColor"
  ...
  const emojiClassName = (index: number) =>
    `w-5 ${rating[index] ? 'text-indigo-600' : 'text-gray-300'} mr-1`;
  const emojis = EMOJIS.map((item, key) => (
    <svg
      aria-hidden="true"
      className={emojiClassName(key)}
  ...
  const number = (num: number) => (
    <span
      className={`text-xl italic font-bold ${
        rating[num - 1] ? 'text-indigo-600' : 'text-gray-300'
      } mr-2`}
    >
      {num}
    </span>
  ...
  const handleRating = (index: number, event: any) => {
    event.preventDefault();
    const newRating = INITIAL_RATING.map((item, key) => key === index);
    setRating(newRating);
  };

  const ratingArray = isEmoji ? emojis : numbers;
  return (
    <ul className="flex justify-center">
      {ratingArray.map((element, key) => (
        <li key={key}>
          <button
            onClick={event => {
              if (!noChange) {
                handleRating(key, event);
              }
            }}
          >
            {element}
